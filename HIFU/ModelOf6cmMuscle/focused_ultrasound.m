...
% define the grid parameters;simulation space is defined as
% 490*490*40;actual physical size is 220mm*220mm*18mm
...
%--------------------------------------------------------------%
% define the SOS of the propagation medium声速模型
%--------------------------------------------------------------%
medium.sound_speed = waterSpeed*ones(Nx, Ny);  % [m/s]

a = makeDisc(Nx, Ny, Nx/2, Ny/2, muscleRadius/dx);%中心在Nx/2和Ny/2的肌肉模型（大圆）
medium.sound_speed(a == 1) = muscleSpeed; %将整个网格中a=1的部分都改成肌肉的声速，最后产生了一个不同位置不同声速的三维图像（不同声速即不同的介质  ）
...
%--------------------------------------------------------------%
% define the density of the propagation medium密度模型
%--------------------------------------------------------------%
...
freq = 1.5e6;             % [Hz]
amp = 50e3;           % [Pa] 150e3
ppw = muscleSpeed / (freq * dx); % points per wavelength一个波长占多少网格
cfl = 0.3;                              % cfl number
...
source.p_mask = zeros(Nx,Ny,Nz);%发射单元掩膜
load('Gridx_Pos.mat');
load('Gridy_Pos.mat');
num_trans = 256;%发射单元数量
for i = Nz/2-10:2:Nz/2+10%步长为2，20-40
    for j = 1:num_trans
